...
def organize_raw_data(portfolio, profile, transcript,dropna=True):
    ...
    portfolio['duration'] = portfolio['duration'].progress_apply(lambda x: x*24)
    ...
    if dropna == True:
        ...
        print("    ...Droping age, gender, and income n/a from profile...")
...
def main():
    ...
    if len(sys.argv) == 5:
        ...
        # Merge offers with portfolio and profile
        ...
        t_matched = t_matched.merge(portfolio[port_merge_col], on='offer_id')
        t_matched.rename(columns={'reward_x':'reward_rcvd', 'reward_y':'reward_offd'}, inplace=True)

        #Other
        t_matched.drop(columns='amount', inplace=True)
        t_matched.rename(columns={'event_offer completed':'event_offer_completed', \
            'event_offer received':'event_offer_received', 'event_offer viewed':'event_offer_viewed'}, \
            inplace=True)
        ...
        print('Saving data...\n    DATABASE: {}'.format(pickle_filepath))
        t_matched.to_pickle(pickle_filepath)
